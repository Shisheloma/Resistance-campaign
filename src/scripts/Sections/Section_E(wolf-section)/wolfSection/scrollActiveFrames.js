...
export const scrollActiveFrames = scroll => {
    if (scroll > keyframes[0]) { 
        ...
        wolf.style.transform = `translateY(${dvhString(wolfY)})`;   
        return
    }
    ...
    if (scroll > keyframes[4]) {  
        ...
        evil.style.transform = `translate(${dvhString(evilX)}, ${dvhString(evilY)}) rotate(${evilAngle}deg)`; 
        return
    }
    ...
    if (scroll > keyframes[6]) { 
        ...
        panties.style.transform = `translate(-50%, ${dvhString(pantiesY)})`;  
        return
    }
    ...
    if (scroll > keyframes[9]) {  
        ...
        strokeOutput.style.transform = `translate(-50%, ${dvhString(strokeY)})`;

        const textY = 0.7 * (keyframes[8] - scroll) - 100;
        textBottom.classList.add('active');
        textBottom.style.transform = `translate(-50%, ${dvhString(textY)})`; 
